...
- **Models**: `User` (cognitoSub, name, email) and `Password` (userId, title, username, encrypted password, website, notes, category). Both UUID primary keys.
...
- **State**: Redux Toolkit. idToken in sessionStorage, refreshToken in localStorage, masterPassword in sessionStorage (tab-scoped).
